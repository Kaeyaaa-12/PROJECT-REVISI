...
@section('content')
    <h1 class="text-4xl font-bold text-center mb-8">Edit Aksesoris</h1>

    <div class="bg-gray-800 p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
        <form action="{{ route('admin.aksesoris.update', $accessory) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="name" class="block text-gray-300 mb-2">Nama Aksesoris</label>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="category" id="category"
                    class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white"
                    value="{{ old('category', $accessory->category) }}" required>
            </div>
            <div class="mb-4">
                <label for="stock" class="block text-gray-300 mb-2">Stok</label>
                <input type="number" name="stock" id="stock"
                    class="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white"
                    value="{{ old('stock', $accessory->stock) }}" required>
            </div>
            <div class="mb-6">
